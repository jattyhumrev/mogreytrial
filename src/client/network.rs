...
impl NetworkClient {
    ...
    pub async fn receive_frame(&mut self) -> Result<Vec<u8>> {
        ...
        let header_read = timeout(self.connection_timeout, stream.read_exact(&mut header)).await;
        if let Err(_) = header_read {
            debug!("Frame header read timed out after {:?}", self.connection_timeout);
            return Err(HvncError::Connection(format!(
                "Frame header read timeout after {:?}", self.connection_timeout
            )));
        }
        if let Err(e) = header_read.unwrap() {
        ...
        if header[0] != 0xFF || header[1] != 0xD8 {
            warn!("Invalid frame magic bytes: {:02X} {:02X}", header[0], header[1]);
        ...
        if frame_length == 0 {
            warn!("Frame length is zero");
            return Err(HvncError::Connection("Frame length is zero".to_string()));
        }
        
        const MAX_FRAME_SIZE: usize = 10 * 1024 * 1024; // 10MB max frame size
        if frame_length > MAX_FRAME_SIZE {
            warn!("Frame length {} exceeds maximum {}", frame_length, MAX_FRAME_SIZE);
            return Err(HvncError::Connection(
        ...
        let data_read = timeout(self.connection_timeout, stream.read_exact(&mut frame_data)).await;
        if let Err(_) = data_read {
            warn!("Frame data read timed out after {:?}", self.connection_timeout);
            return Err(HvncError::Connection(format!(
                "Frame data read timeout after {:?}", self.connection_timeout
            )));
        }
        if let Err(e) = data_read.unwrap() {
        ...
        if frame_data.len() < 2 || frame_data[0] != 0xFF || frame_data[1] != 0xD8 {
            warn!("Received data is not valid JPEG (size: {})", frame_data.len());
            return Err(HvncError::Connection(
                "Received data is not valid JPEG".to_string()
            ));
        }
        
        debug!("Frame received successfully: {} bytes", frame_data.len());
        Ok(frame_data)
    }
