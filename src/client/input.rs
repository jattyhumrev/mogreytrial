...
lazy_static::lazy_static! {
    static ref INPUT_DEBUG: Mutex<Option<std::fs::File>> = Mutex::new(None);
}

fn init_debug_log() {
    let mut debug_log = INPUT_DEBUG.lock().unwrap();
    if debug_log.is_none() {
        if let Ok(file) = OpenOptions::new()
            .create(true)
            .write(true)
            .truncate(true)
            .open("input_debug_client.log") {
            *debug_log = Some(file);
        }
    }
}
...
fn log_input_debug(message: &str) {
    ...
    // Log to file
    if let Ok(mut debug_log) = INPUT_DEBUG.lock() {
        if let Some(ref mut file) = *debug_log {
            let _ = writeln!(file, "{}", full_message);
            let _ = file.flush();
        }
    }
    
    // Also print to console with clear prefix
...
impl InputCapture {
    ...
    pub fn new() -> Self {
        init_debug_log();
        log_input_debug("InputCapture initialized");
        Self {
            last_mouse_pos: None,
    ...
    pub fn update_coordinate_scale(&mut self, client_size: (usize, usize), server_size: (u32, u32)) {
        if client_size.0 > 0 && client_size.1 > 0 && server_size.0 > 0 && server_size.1 > 0 {
            ...
            debug!("🔧 HVNC Coordinate scale updated: client=({}, {}), server=({}, {}), scale=({:.3}, {:.3})", 
    ...
    pub fn capture_events(&mut self, window: &Window) -> Vec<InputEvent> {
        ...
        if !events.is_empty() {
            log_input_debug(&format!("Captured {} total input events", events.len()));
        }
        ...
        if let Some((mouse_x, mouse_y)) = window.get_mouse_pos(MouseMode::Clamp) {
            ...
            if let Some((last_x, last_y)) = self.last_mouse_pos {
                if (scaled_pos.0 - last_x).abs() > 0.5 || (scaled_pos.1 - last_y).abs() > 0.5 {
                    // 🔧 FIX: Use stored coordinate scale for proper translation
                    let server_x = (scaled_pos.0 * self.coordinate_scale.0) as i32;
                    let server_y = (scaled_pos.1 * self.coordinate_scale.1) as i32;
                    
                    debug!("🔍 HVNC Mouse Move: Client({:.1}, {:.1}) -> Server({}, {}), Scale({:.3}, {:.3})", 
                           scaled_pos.0, scaled_pos.1, server_x, server_y,
                           self.coordinate_scale.0, self.coordinate_scale.1);
    ...
    fn capture_mouse_button_events(&mut self, window: &Window, mouse_pos: (f32, f32)) -> Vec<InputEvent> {
        ...
        for (is_pressed, button) in button_mappings.iter() {
            ...
            if *is_pressed && !was_pressed {
                ...
                // 🔧 FIX: Use stored coordinate scale for proper translation
                let server_x = (mouse_pos.0 * self.coordinate_scale.0) as i32;
                let server_y = (mouse_pos.1 * self.coordinate_scale.1) as i32;
                ...
                debug!("💁 HVNC Mouse Click: {:?} pressed at Client({:.1}, {:.1}) -> Server({}, {}), Scale({:.3}, {:.3})", 
                      button, mouse_pos.0, mouse_pos.1, server_x, server_y,
                      self.coordinate_scale.0, self.coordinate_scale.1);
                ...
                debug!("💁 HVNC Mouse Release: {:?} released at Client({:.1}, {:.1}) -> Server({}, {})", 
                      button, mouse_pos.0, mouse_pos.1, server_x, server_y);
            }
        }
    ...
    pub fn capture_keyboard_events(&mut self, window: &Window) -> Vec<InputEvent> {
        ...
        if !current_keys.is_empty() {
            let key_names: Vec<String> = current_keys.iter().map(|k| format!("{:?}", k)).collect();
            log_input_debug(&format!("Keys currently pressed: {}", key_names.join(", ")));
        }
        ...
        for key in &current_keys {
            if !self.pressed_keys.contains(key) {
                if let Some(keycode) = self.key_to_virtual_keycode(*key) {
                    events.push(InputEvent::KeyboardEvent {
                    keycode,
                    pressed: true,
                });
                    log_input_debug(&format!("KEY DOWN: {:?} -> keycode {}", key, keycode));
                    debug!("⌨️ HVNC Key Press: {:?} pressed (keycode: {})", key, keycode);
                } else {
                    log_input_debug(&format!("KEY DOWN: {:?} -> NO KEYCODE MAPPING", key));
                }
            }
        }
        ...
        for key in &self.pressed_keys {
            if !current_keys.contains(key) {
                if let Some(keycode) = self.key_to_virtual_keycode(*key) {
                    ...
                    log_input_debug(&format!("KEY UP: {:?} -> keycode {}", key, keycode));
                    debug!("⌨️ HVNC Key Release: {:?} released (keycode: {})", key, keycode);
                } else {
                    log_input_debug(&format!("KEY UP: {:?} -> NO KEYCODE MAPPING", key));
                }
            }
        }
        ...
        if !events.is_empty() {
            log_input_debug(&format!("Generated {} keyboard events", events.len()));
