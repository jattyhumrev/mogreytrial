...
pub fn compress_to_jpeg(rgb_data: &[u8], width: u32, height: u32, quality: u8) -> Result<Vec<u8>> {
    ...
    trace!(
        "compress_to_jpeg: w={} h={} len={} quality={}",
        width,
        height,
        rgb_data.len(),
        quality
    ...
    // Encode directly from the raw RGB data (3 bytes per pixel)
...
pub fn capture_window_with_desktop_switch(window: WindowHandle, desktop: Option<crate::host::DesktopHandle>) -> Result<Frame> {
    // Delegate to the implementation in capture_extensions to maintain module path expected by server.rs
    crate::host::capture_extensions::capture_window_with_desktop_switch(window, desktop)
}
pub fn capture_window_with_desktop_context(window: WindowHandle, desktop: Option<crate::host::DesktopHandle>) -> Result<Frame> {
    // Attempt window capture with desktop switch first, then fallback to desktop context capture
    match crate::host::capture_extensions::capture_window_with_desktop_switch(window, desktop) {
        Ok(frame) => Ok(frame),
        Err(e) => {
            // Fallback: if desktop provided, try capturing the desktop surface
            if let Some(d) = desktop {
                warn!("Desktop switch capture failed ({}), trying desktop context capture", e);
                crate::host::capture_extensions::capture_desktop_with_context(d)
            } else {
                Err(e)
            }
        }
