...
impl ClientApp {
                    ...
                    match self.handle_frame_reception().await {
                        Ok(frame_received) => {
                            if frame_received {
                                ...
                                // Only reset errors when we actually received and processed a frame
                                connection_errors = 0;
                            } else {
                                debug!("No frame received this tick");
                                // Do not reset connection_errors here; allow error policy to act on repeated failures
                            }
                        }
                        Err(e) => {
                            connection_errors += 1;
                            ...
                            if connection_errors >= MAX_CONNECTION_ERRORS {
                                ...
                                // Reset error counter after a reconnection attempt
                                connection_errors = 0;
                            }
                        }
                    }
    ...
    async fn handle_frame_reception(&mut self) -> Result<bool> {
        match self.network_client.receive_frame().await {
            ...
            Err(e) => {
                // Propagate errors so outer loop can increment counters and attempt reconnection
                warn!("Frame reception error: {}", e);
                Err(e)
            }
        }
    }
